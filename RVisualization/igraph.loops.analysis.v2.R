...
gene2allcommunity.stats.1 <- table(gene2allcommunity[,c(5,16)])
gene2allcommunity.stats.2 <- table(gene2allcommunity[,c(5,17)])
gene2allcommunity.stats.3 <- table(gene2allcommunity[,c(5,18)])
gene2allcommunity.stats <- rbind(t(gene2allcommunity.stats.1), t(gene2allcommunity.stats.2), t(gene2allcommunity.stats.3))
...
gene2allcommunity.stats.1 <- table(gene2allcommunity[,c(5,19)])
gene2allcommunity.stats.2 <- table(gene2allcommunity[,c(5,20)])
gene2allcommunity.stats.3 <- table(gene2allcommunity[,c(5,21)])
...
pdf(file = paste(args[1], "barplot.network.compnent.Total.Long.Thick.pdf", sep = "."), width = 6, height = 8)
print(myplot)
dev.off()
...
traindata <- node2edges.attr[Type!="Enh",c(2:9,11:13,15)]
...
node2edges.tsne.2 <- RuntSNE(x = traindata[,5:12], pre = paste(args[1], "node2edges.2", sep = "."))
...
trainD <- data.matrix(gene2communityfeature[,c(2:4,6,9,11:13,15:22)])
...
# PP : PE ------------------------------------------------------------
datatest <- node2edges.attr[,c(8:9,14,17)]
...
zz <- melt(datatest[Type!="Enh",c(3,5:6)], id.vars = "Type")
...
zz <- melt(datatest[Type!="Enh",c(4,5:6)], id.vars = "CpGquantile")
...
# Thick : Thin ------------------------------------------------------------
datatest <- node2edges.attr[,c(2:3,14,17)]
...
# Long : Short ------------------------------------------------------------
datatest <- node2edges.attr[,c(4:5,14,17)]
...
datatest <- node2edges.attr[Type!="Enh",c(2:5,8:9,14,17)]
datatest[,Thick.percetage:=as.numeric(ThickCount)/(as.numeric(ThickCount)+as.numeric(ThinCount))]
datatest[,Long.percetage:=as.numeric(LongCount)/(as.numeric(LongCount)+as.numeric(ShortCount))]
datatest[,PP.percetage:=as.numeric(PPCount)/(as.numeric(PPCount)+as.numeric(PECount))]
