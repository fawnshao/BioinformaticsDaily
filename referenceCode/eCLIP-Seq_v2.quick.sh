...
# cutadapt searches and removes only the sequence as you provide it. 
# The reverse-complement is not searched. 
# This is because usually the two adapters used on both sides can be different from each other. 
# If the adapters are the same ones in your case, you will need to reverse-complement 
# the sequence yourself and provide it to cutadapt with a second -a option. 
# Cutadapt will then remove the best-matching adapter from each read.
for pre in $AllInputs
do
...
# mkdir $rmrep
# # STAR rmRep: Takes output from cutadapt round 2. 
...
# This is controlled by --outFilterScoreMinOverLread --outFilterMatchNminOverLread which by default are set to 0.66. 
# which means that ~2/3 of the total read length (sum of mates) should be mapped.
# So what your output means is that 12% of the reads aligned unniquely, 
# 7.7% aligned but multimapped and then 80% of your reads couldn't align with the above parameters. 
# You can try to reduce these parameters to see how many more reads will be mapped. 
# However, it looks like your data might just be contaminated with that alignment score :((
# --outSAMunmapped Within \
# for pre in $inputfile ${IPA}_X1A ${IPA}_X1B ${IPB}_X2A ${IPB}_X2B
for pre in $AllInputs
do
...
# Barcode_collapse_pe: takes output from STAR genome mapping. 
...
for pre in $AllInputs
do
	barcode_collapse_pe.py --bam ${pre}.rmRep.genome.Aligned.out.bam \
		--out_file ${pre}.rmRep.genome.rmDup.bam --metrics_file ${pre}.rmRep.genome.rmDup.metrics
	picard SortSam INPUT=${pre}.rmRep.genome.rmDup.bam \
		OUTPUT=${pre}.rmRep.genome.sorted.bam VALIDATION_STRINGENCY=SILENT SO=coordinate CREATE_INDEX=true &
done
wait
for pre in $AllInputs
do
	...
	make_bigwig_files.py --dont_flip --bam ${pre}.rmRep.genome.sorted.r2.bam \
		--genome $genomesize --bw_pos ${pre}.rmRep.genome.sorted.r2.norm.pos.bw \
		--bw_neg ${pre}.rmRep.genome.sorted.r2.norm.neg.bw &
done
wait
for pre in $AllInputs
do	
    normalize_bedGraph.py --bg ${pre}.rmRep.genome.sorted.r2.pos.bg \
    	--bam ${pre}.rmRep.genome.sorted.r2.bam > ${pre}.rmRep.genome.sorted.r2.norm.pos.bg &
    normalize_bedGraph.py --bg ${pre}.rmRep.genome.sorted.r2.neg.bg \
    	--bam ${pre}.rmRep.genome.sorted.r2.bam > ${pre}.rmRep.genome.sorted.r2.norm.neg.bg &
done
wait
for pre in $AllInputs
do    
    negBedGraph.py --bg ${pre}.rmRep.genome.sorted.r2.norm.neg.bg > ${pre}.rmRep.genome.sorted.r2.norm.neg.t.bg
	...
	bedGraphToBigWig ${pre}.rmRep.genome.sorted.r2.norm.pos.srt.bg $genomesize ${pre}.rmRep.genome.sorted.r2.norm.pos.bw &
	bedGraphToBigWig ${pre}.rmRep.genome.sorted.r2.norm.neg.t.srt.bg $genomesize ${pre}.rmRep.genome.sorted.r2.norm.neg.bw &
done
wait
...
echo "stats"
grep "Uniquely mapped reads %" $mappingtogenome/*Log.final.out
grep "Number of input reads" *Log.final.out | awk '{print $7}'
grep "Total read pairs processed" $trimadapterdir/*.metrics  | awk '{print $5}'
for f in *rmDup.bam; do echo $f; samtools flagstat $f; echo "======================"; done
